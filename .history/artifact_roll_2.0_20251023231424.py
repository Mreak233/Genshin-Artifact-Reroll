...
def compute_small_weight_equivalent(
    ...
    if return_fraction:
        ...
        base_factor = (float(n_small) * 100.0) / (float(big_base_value) * float(n_big))
...
def preprocess_initial_four(
    ...
    *,
    ...
    shorthand = {"攻":"大攻击","小攻":"小攻击","生":"大生命","小生":"小生命","防":"大防御","小防":"小防御","暴":"暴击","爆":"暴伤","精":"元素精通","充":"充能效率"}
    ...
    if role_use is None:
        raise KeyError(f"Role weights for '{role_name}' not found. Available roles: {list(role_weights_norm.keys())}")
    role_attrs = role_weights_norm[role_use]

    if role_use not in base_stats_norm:
        if verbose:
            print(f"Warning: base stats for '{role_use}' not found; small substat conversions will fallback to 0")
        role_base = {}
    else:
    ...
    for p in initial_keys:
        ...
        if mapped in SMALL_TO_BIG:
            ...
            n_big = float(n_values_norm.get(big_norm, 1.0)) if big_norm in n_values_norm else 1.0
            basekey = SMALL_TO_BASEKEY.get(small)
            base_val = None
            if basekey:
                base_val = role_base.get(basekey)
            small_w = compute_small_weight_equivalent(big_weight, n_small, base_val, n_big=n_big, return_fraction=False)
            resolved[small] = small_w
            if verbose:
                print(f"mapped '{p}' -> '{small}' (small). big='{big_norm}', big_weight={big_weight}, base={base_val}, n_small={n_small}, small_w={small_w}")
        else:
            nm = mapped
    ...
    used_nonzero = sorted([k for k, v in resolved.items() if not isclose(v, 0.0)])
...
def build_support_auto(
    ...
    all_integer = not contains_small and all(abs(round(v) - v) <= tol for v in candidate_vals)
    if all_integer:
        support_int = [int(round(v)) for v in candidate_vals]
        g = gcd_list(support_int) or 1
        support_reduced = [v // g for v in support_int] if g > 1 else support_int
...
def convolve_counts_with_hits(
    ...
    for name in candidate_pool:
        ...
        for t in tiers:
            if used_fractional_flag:
                val_frac = Fraction(str(float(w))).limit_denominator() * Fraction(t) * Fraction(scale)
    ...
    single_items_unique = list(single_count.items())  # [ ((val,hit), multiplicity), ... ]
...
def conditional_replace_counts(
    ...
    for (score_int, h), mult_orig in counts_orig.items():
        ...
        allocations = { v_out: mult_orig * (cnt_internal / total_internal) for v_out, cnt_internal in internal_counts.items() }
    ...
    if sum_final != sum_orig:
        ...
        if verbose:
            print(f"conditional_replace_counts: sum mismatch sum_final={sum_final} sum_orig={sum_orig} diff={diff}; applying correction")
        if diff > 0:
...
def plot_pmf(
    ...
    *,
    ...
    if mode == "none":
        if verbose: print("plot_pmf: mode='none' -> skip")
        return None
    ...
    try:
        ...
        if backend:
            try:
                ...
                if verbose: print(f"plot_pmf: backend set to {backend}")
            except Exception as e:
                if mode == "gui":
                    raise RuntimeError(f"plot_pmf: failed to set backend {backend}: {e}")
                if verbose: print(f"plot_pmf: failed to set backend {backend}, fallback: {e}")
        import matplotlib.pyplot as plt
        import numpy as np
    except Exception as e:
        if mode == "gui":
            raise RuntimeError(f"plot_pmf: unable to init matplotlib for GUI: {e}")
        if verbose: print(f"plot_pmf: matplotlib init failed, switching to save mode: {e}")
        mode = "save"

    x = list(pmf_readable.keys())
    if not x:
        if verbose: print("plot_pmf: empty pmf_readable -> nothing to plot")
        return None
    ...
    try:
        ...
        def update_annot(idx: int):
            ...
            annot.xy = (score, prob)
            annot.set_text(f"score={score:.2f}\nprob={prob:.8f}\ncum={cum:.6f}")
        ...
        if mode == "gui":
            plt.close('all')
            raise
        if verbose: print(f"plot_pmf: plotting failed; will try save fallback: {e}")

    if mode in ("save", "auto"):
        try:
            abs_path = os.path.abspath(filename)
            plt.savefig(filename, bbox_inches="tight")
            if verbose: print(f"PMF plot saved to {abs_path}")
        except Exception as e_save:
            if mode == "save":
                print(f"plot_pmf: failed to save PMF plot: {e_save}")
                plt.close()
                return None
            if verbose: print(f"plot_pmf: save failed, attempt GUI: {e_save}")

    if mode in ("gui", "auto"):
        try:
            if verbose: print("plot_pmf: showing GUI (blocking)...")
            plt.show()
        finally:
            try:
                plt.close()
...
def analyze_roll(
    ...
    support_reduced, scale, gcd_factor, used_fractional_flag = build_support_auto(used, explicit_weights_sub, tiers=tiers, small_set=small_set, tol=tol)
    ...
    if do_replace:
        ...
        forced_set = {fp0, fp1}
        if verbose:
            print("analyze_roll: forced_pair normalized to", forced_set, "min_hits=", min_hits)
            print("analyze_roll: candidate_pool (normalized):", [normalize_key(x) for x in candidate_pool])
        ...
        if verbose:
            print("analyze_roll: A_counts (sample keys):", sorted(A_counts.keys())[:10])
            print("analyze_roll: B_counts (sample keys):", sorted(B_counts.keys())[:10])

        counts_reduced = conditional_replace_counts(counts_orig, A_counts, B_counts, R, min_hits, verbose=verbose)
    else:
        sup_count = support_counts(support_reduced)
        counts_reduced = convolve_counts_sparse(sup_count, R)

    pmf_readable, mean, std, denom = counts_to_pmf_stats(counts_reduced, scale, gcd_factor)

    if inclusive:
        numer = sum(cnt for k_red, cnt in counts_reduced.items() if ((k_red * gcd_factor) / scale) >= threshold)
...
def run_analysis(
    ...
    norm_initial_input = { normalize_key(k): float(v) for k, v in initial_substats.items() }
    norm_current_input = { normalize_key(k): float(v) for k, v in current_stats_values.items() }
    if verbose:
    ...
    if len(initial_keys) != 4:
        raise ValueError("initial_substats must contain exactly four items (四个初始词条)")
    
    current_keys = list(current_stats_values.keys())

    if len(current_keys) != 4:
        raise ValueError("current_stats_values must contain exactly four items (四个初始词条)")
    ...
    initial_score, initial_units = compute_score_units_based(initial_stats_filtered, n_values_norm, explicit_weights_sub, return_units_map=True)
    current_score, current_units = compute_score_units_based(current_stats_filtered, n_values_norm, explicit_weights_sub2, return_units_map=True)
    ...
    print("Used:", res["used"])
    print("scale:", res["scale"], "gcd_factor:", res["gcd_factor"], "fractional_path:", res["used_fractional_path"])
    print("Initial score:", res["initial_score"])
    print("Current score:", res["current_score"])
    print("Threshold:", res["threshold"])
    print("R:", res["R"])
    print("Probability (exact): {}/{} ≈ {:.6%}".format(res["numerator"], res["denominator"], res["probability_float"]))
    print("Mean added score ≈ {:.6f}, std ≈ {:.6f}".format(res["mean_added_score"], res["std_added_score"]))
    print("min_real:", res["min_real"], "max_real:", res["max_real"])

    if plot_mode != "none":
        plot_pmf(res["pmf_readable"], res["threshold"], res["R"], mode=plot_mode, backend=plot_backend, filename=plot_filename, verbose=verbose)
...
if __name__ == "__main__":
    ...
    role = "菲谢尔"
    init_four = {"攻":4.1, "精":21, "充":6.5,"爆":7}
    current = {"小攻":14, "攻":4.1, "暴":21.4,"小防":16}
