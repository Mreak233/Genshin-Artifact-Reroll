...
def print_summary(res: Dict[str, Any]) -> None:
    ...
    rng = f"{res['min_real'] * scale_x:.2f}..{res['max_real'] * scale_x:.2f}"
    print("=== Analysis summary ===")
    print(f"Used stats: {res['used']}")
    print(f"Initial score: {(float)init * scale_x}    Current score: {(float)cur * scale_x}    Threshold: {(float)thr * scale_x}")
    print(f"Support range (min..max): {rng}    R: {res['R']}")
    print(f"Probability (exact): {prob_frac}  (≈ {prob_pct})")
    print(f"Mean added score: {mean * scale_x}    Std: {std * scale_x * scale_x}")
    print("========================")
